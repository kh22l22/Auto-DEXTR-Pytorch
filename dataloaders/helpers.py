...
def calc_extreme_points(img):
    ...
    cv2.circle(img, (Left[0], Left[1]), 5, (0, 0, 255), -1)
    cv2.circle(img, (Right[0], Right[1]), 5, (0, 255, 0), -1)
    cv2.circle(img, (Top[0], Top[1]), 5, (255, 0, 0), -1)
    cv2.circle(img, (Bot[0], Bot[1]), 5, (255, 255, 0), -1)

    cv2.imwrite('extreme_points.png', img)

    extreme_points_ori = [Right, Left, Top, Bot]

    return extreme_points_ori


def calc_extreme_points_Depth(img):

    # grey_img = cv2.cvtColor(img.copy(), cv2.COLOR_BGR2GRAY)
    #print(img)
    img = np.uint8(img*255)
    edges = cv2.Canny(img, 100, 105)
    indices = np.where(edges != [0])
    coordinates = zip(indices[0], indices[1])
...
def calc_extreme_points_NLFD(img, prob):


    ret, mask = cv2.threshold(prob, 10, 255, cv2.THRESH_BINARY)
    ...
    filtered_mask2 = np.zeros_like(labels)
    filtered_mask2[labels == max_label] = 1

    # filtered_mask2 = np.array(filtered_mask2, dtype=np.uint8)
    # filtered_mask2 = np.expand_dims(filtered_mask2, axis=2)

    mask[np.where(filtered_mask2 == 0)] = 0
    ...
    edges = cv2.Canny(img, 100, 400)
    print(edges)
    #indices = np.where(edges != [0] & mask == 255 )
    indices = np.where(edges != [0])
    coordinates = zip(indices[0], indices[1])
    ...
    print (mask.shape, img.shape, mask[0,0])
